    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-3">
        <div class="d-flex align-items-center">
            ...
            <div>
                ...
                <p class="text-muted small mb-0">Kelola data siswa dan akun login ujian CBT.</p>
            </div>
        </div>
        
        <div class="d-flex gap-2">
            {{-- Tombol Import Excel --}}
            <button type="button" class="btn btn-excel-outline fw-bold rounded-pill px-4 py-2 shadow-sm transition-3d d-flex align-items-center" data-bs-toggle="modal" data-bs-target="#modalImportExcel">
                <i class="bi bi-file-earmark-excel-fill me-2"></i> Import Excel
            </button>

            {{-- Tombol Tambah Siswa --}}
            <button class="btn btn-success rounded-pill px-4 py-2 shadow-sm fw-bold border-0 transition-3d d-flex align-items-center" data-bs-toggle="modal" data-bs-target="#addSiswaModal">
    ...
    <div class="card border-0 shadow-sm rounded-4 p-3 mb-4 bg-white">
        <form action="{{ route('admin.siswas.index') }}" method="GET" id="filterForm" class="m-0">
            <div class="row g-2 align-items-center">
                <div class="col-auto">
                    <label class="form-label small fw-bold text-muted text-uppercase mb-0 me-2"><i class="bi bi-funnel-fill text-primary me-1"></i> Filter Kelas:</label>
                </div>
                <div class="col-md-4 col-lg-3">
                    <select name="kelas" class="form-select bg-light border-0 shadow-none cursor-pointer rounded-pill px-3" onchange="document.getElementById('filterForm').submit()">
                ...
                {{-- Tombol Reset Filter (Muncu otomatis kalau filter sedang aktif) --}}
                @if(request('kelas'))
                <div class="col-auto">
                    <a href="{{ route('admin.siswas.index') }}" class="btn btn-sm btn-light rounded-pill text-danger px-3 shadow-sm transition-3d">
            ...
            <table class="table table-hover align-middle mb-0 custom-table">
                <thead class="bg-light text-muted small fw-bold text-uppercase" style="letter-spacing: 0.5px;">
                    <tr>
                        ...
                        <th class="py-3 border-0" style="width: 20%;">Email</th>
                ...
                <tbody>
                    ...
                    <tr class="transition-3d-row">
                        ...
                        <td>
                            <div class="text-muted small"><i class="bi bi-envelope me-1"></i> {{ $s->user->email }}</div>
                        </td>
                        <td class="text-end pe-4">
                            <div class="d-flex justify-content-end gap-2">
                                ...
                                {{-- Form Hapus --}}
                                <form id="delete-form-{{ $s->id }}" action="{{ route('admin.siswas.destroy', $s->id) }}" method="POST" class="d-none">
                                    @csrf 
                                    @method('DELETE')
                                </form>
                            </div>
                    ...
                    <tr>
                        <td colspan="6" class="text-center py-5">
                            <div class="p-4">
                                ...
                                <p class="text-muted small mb-0">Silakan tambahkan siswa secara manual atau import via Excel.</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
        ...
        <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
            <div class="modal-header bg-white border-bottom pt-4 px-4 pb-3">
                <div class="d-flex align-items-center">
                    ...
                    <h5 class="modal-title fw-bold text-dark mb-0" style="letter-spacing: 0.5px;">TAMBAH SISWA BARU</h5>
                </div>
            ...
            <form action="{{ route('admin.siswas.store') }}" method="POST">
                ...
                <div class="modal-body p-4" style="background-color: #f8fafc;">
                    <div class="mb-3">
                        ...
                        <input type="text" name="nama" class="form-control bg-white border custom-input py-2 shadow-none" placeholder="Masukkan nama siswa" required>
                    </div>
                    <div class="row g-3 mb-3">
                        <div class="col-md-6">
                            <label class="form-label small fw-bold text-muted text-uppercase mb-2">NISN</label>
                            <input type="text" name="nisn" class="form-control bg-white border custom-input py-2 shadow-none" placeholder="Nomor NISN" required>
                            <small class="text-muted mt-1 d-block" style="font-size: 0.75rem;"><i class="bi bi-info-circle me-1"></i>Akan menjadi password default.</small>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label small fw-bold text-muted text-uppercase mb-2">Pilih Kelas</label>
                            <select name="kelas_id" class="form-select bg-white border custom-input py-2 shadow-none cursor-pointer" required>
                    ...
                    <div class="mb-2">
                        <label class="form-label small fw-bold text-primary text-uppercase mb-2"><i class="bi bi-envelope-at me-1"></i> Email</label>
                        <input type="email" name="email" class="form-control bg-white border custom-input py-2 shadow-none" placeholder="contoh@siswa.com" required>
                    </div>
                </div>
            ...
            <form id="editSiswaForm" method="POST">
                ...
                <div class="modal-body p-4" style="background-color: #f8fafc;">
                    ...
                    <div class="row g-3 mb-3">
                        <div class="col-md-6">
                            <label class="form-label small fw-bold text-muted text-uppercase mb-2">NISN</label>
                            <input type="text" name="nisn" id="edit_nisn" class="form-control bg-white border custom-input py-2 shadow-none focus-warning" required>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label small fw-bold text-muted text-uppercase mb-2">Pilih Kelas</label>
                            <select name="kelas_id" id="edit_kelas_id" class="form-select bg-white border custom-input py-2 shadow-none cursor-pointer focus-warning" required>
                                <option value="">-- Pilih Kelas --</option>
                                @foreach($kelases as $kelas)
                                    <option value="{{ $kelas->id }}">{{ $kelas->nama_kelas }}</option>
                                @endforeach
                    ...
                    <div class="row g-3 mb-2">
                        ...
                        <div class="col-md-6">
                            <label class="form-label small fw-bold text-danger text-uppercase mb-2"><i class="bi bi-shield-lock me-1"></i> Password Baru</label>
                            <input type="password" name="password" class="form-control bg-white border custom-input py-2 shadow-none focus-warning" placeholder="Isi jika ingin reset">
                            <small class="text-muted mt-1 d-block" style="font-size: 0.75rem;"><i class="bi bi-info-circle me-1"></i>Kosongkan jika tidak ingin diubah.</small>
                        </div>
                    </div>
                </div>
                <div class="modal-footer bg-white border-top p-3 px-4">
        ...
        <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
            <div class="modal-header bg-white border-bottom pt-4 px-4 pb-3">
                <div class="d-flex align-items-center">
                    ...
                    <h5 class="modal-title fw-bold text-dark mb-0" style="letter-spacing: 0.5px;">IMPORT DATA SISWA</h5>
                </div>
                <button type="button" class="btn-close shadow-none" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="{{ route('admin.siswas.import') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="modal-body p-4" style="background-color: #f8fafc;">
                    <div class="alert alert-info py-3 px-4 small border-0 rounded-4 mb-4 d-flex align-items-start shadow-sm">
                        <i class="bi bi-info-circle-fill fs-5 me-3 text-info"></i> 
                        <div>
                            Pastikan format Excel Anda memiliki baris judul kolom: <br>
                            <b class="text-dark">nama, nisn, kelas, email</b> pada baris pertama.
                        </div>
                    </div>
                    <div class="mb-2">
                        <label class="form-label small fw-bold text-muted text-uppercase mb-2">Upload File Excel (.xlsx)</label>
                        <input type="file" name="file_excel" class="form-control bg-white border custom-input py-2 shadow-none" accept=".xlsx, .xls, .csv" required>
                    </div>
                </div>
                <div class="modal-footer bg-white border-top p-3 px-4">
                    <button type="button" class="btn btn-light rounded-pill px-4 fw-medium text-secondary" data-bs-dismiss="modal">Batal</button>
                    <button type="submit" class="btn text-white rounded-pill px-5 fw-bold shadow-sm d-flex align-items-center transition-3d" style="background-color: #107c41;">
...
<style>
    ...
    /* Button Excel */
    .btn-excel-outline { background-color: #ffffff; color: #107c41; border: 2px solid #107c41; }
    .btn-excel-outline:hover { background-color: #107c41; color: #ffffff; border-color: #107c41; box-shadow: 0 8px 15px rgba(16, 124, 65, 0.2) !important; }
</style>
@endsection
