...
@section('content')
            ...
            <div class="col-md-6 col-lg-4">
                <div class="card border-0 shadow-sm rounded-4 h-100 transition-hover overflow-hidden">
                    <div class="card-body p-4">
                        <span
                            class="badge bg-success-subtle text-success mb-3 px-3 rounded-pill">{{ $e->subject->name }}</span>
                        ...
                        <div class="d-grid">
                            ...
                            @elseif($session && !$session->completed_at)
                                <a href="{{ route('siswa.exams.show', $e->id) }}"
                                    class="btn btn-warning rounded-pill fw-bold py-2 shadow-sm">
                                    Lanjutkan Ujian <i class="bi bi-play-fill ms-1"></i>
                                </a>
                            @else
                                <button class="btn btn-success rounded-pill fw-bold py-2 shadow-sm"
                                    onclick="persiapanUjian('{{ $e->id }}', '{{ $e->title }}')">
                                    Kerjakan Sekarang <i class="bi bi-arrow-right ms-1"></i>
                                </button>
                            @endif
                        </div>
                    </div>
            ...
            <div class="modal-content border-0 shadow-lg rounded-4">
                <form id="formToken" method="POST">
                    ...
                    <div class="modal-footer border-0 pt-0 pb-4 justify-content-center gap-2">
                        ...
                        <button type="submit" class="btn btn-success rounded-pill px-5 fw-bold shadow">Mulai
                            Sekarang</button>
                    </div>
                </form>
            </div>
    ...
    <style>
        .transition-hover:hover {
            ...
            transition: 0.3s;
            box-shadow: 0 1rem 3rem rgba(16, 185, 129, 0.1) !important;
        }
